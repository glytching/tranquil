...
public class JacksonMappingProviderTest {

  private MappingProvider sut;

  @BeforeEach
  public void setup() {
    sut = new JacksonMappingProvider();
  }

  @Test
  public void canDeserializeSimpleJson() throws Exception {
    assertSimpleJsonDeserialization(() -> sut.deserialize(SIMPLE_JSON));

    assertSimpleJsonDeserialization(() -> sut.deserialize(toInputStream(SIMPLE_JSON), "UTF-8"));
  }

  @Test
  public void canDeserializeJsonArray() throws Exception {
    assertJsonArrayDeserialization(() -> sut.deserialize(JSON_ARRAY));

    assertJsonArrayDeserialization(() -> sut.deserialize(toInputStream(JSON_ARRAY), "UTF-8"));
  }
  ...
  public void canDeserializeJsonWithSingleArrayAttribute() throws Exception {
    assertJsonArrayWithSingleArrayAttributeDeserialization(
        () -> sut.deserialize(JSON_WITH_SINGLE_ARRAY_ATTRIBUTE));

    assertJsonArrayWithSingleArrayAttributeDeserialization(
        () -> sut.deserialize(toInputStream(JSON_WITH_SINGLE_ARRAY_ATTRIBUTE), "UTF-8"));
  }

  @Test
  public void canDeserializeJsonWithMixtureOfArrayAndNonArrayAttributes() throws Exception {
    assertJsonArrayWithMixedAttributesDeserialization(() -> sut.deserialize(COMPLEX_JSON));
    assertJsonArrayWithMixedAttributesDeserialization(
        () -> sut.deserialize(toInputStream(COMPLEX_JSON), "UTF-8"));
  }
  ...
  @ParameterizedTest
  @ValueSource(strings = {SIMPLE_JSON, JSON_ARRAY, COMPLEX_JSON, JSON_WITH_SINGLE_ARRAY_ATTRIBUTE})
  ...
  public void canSerializeToABespokeType() {
    List<Map<String, Object>> deserialized = sut.deserialize(SIMPLE_JSON);
  ...
  public void canSerializeToABespokeCollection() {
    List<Map<String, Object>> deserialized = sut.deserialize(JSON_ARRAY);
  ...
  public void canSerializeJsonArray() {
    ...
    assertThat(sut.serialize(deserialized), is(SIMPLE_JSON));
  }
  ...
  private void assertSimpleJsonDeserialization(Callable<List<Map<String, Object>>> callable)
    ...
    assertThat(deserialized.get(0), is(getTapItemAsMap()));
  }
